...
async function main() {
  ...
  const amount = await StakedAuroraVaultContract.balanceOf(VAULT_ADMIN_ACCOUNT.address);

  const request02 = await StakedAuroraVaultContract
    .connect(VAULT_ADMIN_ACCOUNT)
    .redeem(amount, VAULT_ADMIN_ACCOUNT.address, VAULT_ADMIN_ACCOUNT.address);
